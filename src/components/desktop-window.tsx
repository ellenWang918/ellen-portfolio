...
export function DesktopWindow({ title, sidebar, children, onClose }: DesktopWindowProps) {
  return <section className="desktop-window">
    <header className="desktop-window__titlebar"><h2>{title}</h2><div className="desktop-window__controls"><button type="button" aria-label="Close window" onClick={onClose} /><span aria-hidden="true" /><span aria-hidden="true" /></div></header>
